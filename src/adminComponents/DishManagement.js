...
const DishTable = () => {
  ...
  const [dish, setDish] = useState([
    {
      dishName: "",
      ingredients: [
        { id: "", name: "", cost: 0, quantity: 0, category: "", unit: "" },
      ],
  ...
  // Helper function to check if ingredients are in default state
  // Helper function to check if ingredients are in default state
  ...
  return (
    <div className="flex flex-col justify-items-center p-4 mb-20 ml-14 mr-2 overflow-x-hidden flex-grow">
      <div className="flex justify-between">
        ...
        <Link to={`/dishlist`}>
          <h3 className="font-bold mb-2 text-thirtiary underline">Dish list</h3>
        </Link>
      </div>
                    ...
                    {d.recipes.map((recipe, recipeIndex) => (
                      <li key={recipeIndex} className="flex items-center mb-2">
                        ...
                        <input
                          ...
                          className="w-1/4 px-2 py-1 mr-2 border border-gray-300 rounded"
                        />
                        <label>{recipe.quantity} unit</label>
                        <button
